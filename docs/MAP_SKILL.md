...
The bundled Node auditor imports a supplied bundle through StateWork into memory, or a portable directory package into a fresh temporary SQLite database removed afterward. It does not open the live database. It checks discovered-link accounting, requirement-to-item mappings, supporting quotes, completion evidence and every task's backend handoff with external access disabled. It cannot discover requirements the researcher failed to inventory or establish the semantic correctness of a procedure. Review the original indexes and perform the skill's dry run as well.
...
The real candidate additionally demonstrated overgeneralized step checks: a final receipt or acceptance outcome was repeated before delivery, and a local setup task inherited a generic submission step. The skill now requires checks that can be observed at each step, preserves permitted alternative routes, and avoids duplicate source buttons and source-recovery entries. A separate [route source pack](../examples/map-skill-route-sources/index.md) exercises local setup, an experienced worker's drawing route, delivery evidence order and repeated source anchors. These targeted examples are regression probes, not a representative sample of all professions.

Baseline comparison also found omitted earlier sent enquiries, verified against the original messages. The skill now explicitly expands a seed sender into relevant sent mail, replies and historical records without assuming that an enquiry or acknowledgement means an accepted outcome. The [fictional mailbox](../examples/map-skill-history-sources/mailbox/index.json) provides a small repeatable history probe. The fresh real candidate should not replace an existing personal map without reconciling that history and its remaining source/packet gaps.

The file audit found incidental course navigation images mixed with useful originals, while substantive course files were omitted by the initial storage limit. The [file research workflow](../.agents/skills/statework-map/references/files.md) now requires source-supported file-purpose decisions, preserves procedural images even when they contain logos, and reuses exact byte hashes without discarding source relationships. Current storage supports 128 MiB/file and defaults to 2 GiB/workspace. Larger collections use portable directory packages; legacy inline JSON bundles retain their smaller limits. Originals keep their full quality.

Use this skill for researched map construction and reviewed handoffs. A discovered-but-blocked requirement is useful evidence, not executable work. Essential inputs beyond the installed storage bounds require a suitable integration or an explicitly inspected task-specific derivative. Physical app operation, source-system permissions, unstated requirements and professional judgment remain outside what a map audit can guarantee. A small test cannot establish universal completeness or autonomous completion of every occupation.
