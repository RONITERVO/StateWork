...
The SQLite adapter uses `BEGIN IMMEDIATE`, foreign keys, WAL, a 5-second busy timeout and `synchronous=FULL`. Snapshot, receipt, event and attached file bytes are committed together. A callback failure rolls all of them back. Multiple local processes serialize through SQLite. A database created with a newer schema is rejected. Migration 2 is transactional and recorded in `PRAGMA user_version`; upgrading version 1 first creates a consistent adjacent backup.
...
6. **Organization rules:** inject a synchronous trusted `WorkPolicy` into `WorkService`. It can enforce review roles or required domain metadata inside command transactions. Workspace JSON and captured instructions never install policies.
7. **Original files:** implement the optional transactional asset-store capability to preserve exact bytes separately from snapshots. Include file identities, digest verification and quota handling. Metadata-only adapters expose missing files explicitly.
...
The optional `WorkState.instructions` container preserves immutable source captures, original-file metadata and packet content revisions. Host-attributed review and result checks use the same atomic commands, receipts and history. Legacy packets remain sequential. Connected execution adds explicit edges, parallel work, observed branch choices, successful finish points and result files. The SDK validates structured input; core computes procedure gaps, context staleness and worker-specific readiness. A reviewed procedure and checked active route reaching a declared successful finish are required for Done. File-capable hosts also enforce required byte availability. See [connected work contracts and limits](CONNECTED_WORK.md).
