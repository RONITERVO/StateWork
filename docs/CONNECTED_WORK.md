...
Limits are explicit: 64 MiB per file, 256 MiB of unique file bytes per workspace, 2,000 file identities, 100 actions and requirements, 100 outputs and 20 choices per decision. Portable JSON base64 encoding increases transfer size; the import route is bounded at 380 MiB. The browser requires memory for the complete transfer. Large organizational document stores should implement a storage/hosting adapter rather than raise limits without workload testing.
