...
- Versioned instructions and immutable source captures with exact citations, prerequisites, explicit gaps, human review, ordered result checks and durable completion records.
- Screen, keyboard, phone, print/PDF and room/VR packet controls, local PDF/DOCX/text extraction and explicit public-page capture.
- Optional signed-in local Codex draft adapter, bounded/cancellable jobs and source-bundle research handoff; complete manual fallback.
- SDK, REST, OpenAPI, JSON Schema, CLI and MCP instruction contracts; browser snapshot import raised to 16 MiB for source-rich workspaces.
