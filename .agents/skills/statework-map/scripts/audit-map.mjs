#!/usr/bin/env node
// Source/worker audit. Original data is read only; imports use isolated storage.
...
export async function auditMap(sdk, bundle, ledger) {
  ...
  try {
    ...
    return auditImportedMap(work, 'map-audit', ledger, {
...
export function auditPackage(sdk, node, directory, ledger) {
  const manifestPath = join(resolve(directory), 'manifest.json');
  if (statSync(manifestPath).size > node.FILE_PACKAGE_MANIFEST_LIMIT)
    throw new Error('Package manifest exceeds the installed package size limit.');
  const manifestBytes = readFileSync(manifestPath);
  const temporaryRoot = resolve(tmpdir());
  const temporary = mkdtempSync(join(temporaryRoot, 'statework-map-audit-'));
  let service;
  try {
    service = new sdk.WorkService(new node.SqliteStore(join(temporary, 'audit.sqlite')));
    const work = service.connect('map-audit-fresh-worker');
    node.importFilePackage(work, directory, { id: 'map-audit', title: 'Isolated package audit' });
    if (!readFileSync(manifestPath).equals(manifestBytes))
      throw new Error('Package manifest changed during audit.');
    return auditImportedMap(work, 'map-audit', ledger, {
      packageManifestSha256: createHash('sha256').update(manifestBytes).digest('hex'),
    });
  } finally {
    service?.close();
    if (!temporary.startsWith(join(temporaryRoot, 'statework-map-audit-')))
      throw new Error('Unexpected temporary audit directory.');
    rmSync(temporary, { recursive: true, force: true });
  }
}

export function auditImportedMap(work, workspaceId, ledger, provenance) {
  const issues = [];
  ...
  const state = work.snapshot(workspaceId);
  ...
  for (const requirement of ledger.requirements) {
    if (!requirement?.id || requirementIds.has(requirement.id))
      add('REQUIREMENT_ID', requirement?.id ?? '', 'Missing or duplicate requirement ID.', 'error');
    requirementIds.add(requirement?.id);
    if (!requirement) continue;
    const source = ledger.sources.find((s) => s?.id === requirement.sourceId);
    if (!source)
      add('REQUIREMENT_SOURCE', requirement.id, 'Requirement source is not inventoried.', 'error');
  ...
  for (const task of tasks) {
    ...
    if (!scope || !['required', 'chosen', 'optional', 'unresolved'].includes(scope.classification))
      add(
        'TASK_SCOPE',
        task.id,
    ...
    const handoff = work.handoff(workspaceId, task.id, { externalAccess: false });
  ...
  return {
    ...
    counts: {
      ...
      requirementsExcluded: ledger.requirements.filter((r) => r.disposition === 'excluded').length,
...
async function main() {
  ...
  if (!args.length || args.includes('--help')) {
    process.stdout.write(
      'audit-map --app <built-checkout> (--bundle <bundle.json> | --package <directory>) --ledger <research.json> --out <new-report.json>\nNo live database is opened. Packages use a fresh temporary SQLite database removed after audit; bundles use memory. Exit 0: no mechanical findings; 2: findings; 1: invalid input/error. Semantic review is always required.\n',
    );
    return;
  }
  const options = {};
  for (let i = 0; i < args.length; i += 2) {
    if (
      !['--app', '--bundle', '--package', '--ledger', '--out'].includes(args[i]) ||
      !args[i + 1] ||
      options[args[i]]
    )
  ...
  if (
    ...
    !options['--ledger'] ||
    !options['--out'] ||
    Boolean(options['--bundle']) === Boolean(options['--package']) ||
    Object.keys(options).length !== 4
  )
    throw new Error('Use --app, --ledger, --out and exactly one of --bundle or --package.');
  ...
  const report = options['--package']
    ? auditPackage(
        sdk,
        await import(
          pathToFileURL(join(resolve(options['--app']), 'packages/node/dist/index.js')).href
        ),
        resolve(options['--package']),
        ledger,
      )
    : await auditMap(sdk, JSON.parse(readFileSync(resolve(options['--bundle']), 'utf8')), ledger);
