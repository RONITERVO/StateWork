...
Record the rebuild scope explicitly. `ForceRebuild3(true)` rebuilds only the top assembly; `false` includes its subassemblies. If a top-only refresh returns but a nested linkage remains unsolved, inspect that distinction before changing correct geometry. A justified full-hierarchy refresh needs its own bounded result and unchanged-pose checks. It can dirty dependencies, so preserve the working set and handle their save flags separately.
