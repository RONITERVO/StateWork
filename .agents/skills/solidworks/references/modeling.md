...
Confirm the actual active configuration before interpreting measurements. A configuration activation request may return false when that configuration is already active; inspect its identity before deciding the outcome. For every required configuration, check mate suppression, configuration-specific driving values, component resolution and transforms. An error-free mate can still be suppressed, and editing a mate definition can affect more than the currently intended configuration.
