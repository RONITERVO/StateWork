...
Verify an exploded presentation through its actual step members, distances, transforms and a visible preview. A successful creation call, named view or nonempty step list can still describe an unchanged assembly: a native initial step may have zero distance and identity transforms. Preserve that result, add the required presentation offsets to the existing view in a new candidate, and verify the separated view and restored assembled state after saving and reopening. Do not repeat view creation merely because the first view lacks useful separation.
